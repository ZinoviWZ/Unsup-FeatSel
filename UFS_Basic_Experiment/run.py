...
from FeatureSelection import UFS_algorithms, AutoSpearman_algorithms, CFS_algorithms, LS_algorithms, MCFS_algorithms, \
...
def save_selected_feature(fs_name, selected_idx):
    ...
    path = os.path.join(path_idx, fs_name + '.csv')
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, 'a') as file:
        print(selected_idx.astype(int))
        np.savetxt(file, selected_idx.reshape((1,-1)), delimiter=',', fmt='%d')

def save_evalution_measure(evalution_measure_lr, evalution_measure_rf, evalution_measure_sc, evalution_measure_kmeans, evalution_measure):
    path2 = os.path.join(results_dir, 'RQ2')
    path3 = os.path.join(results_dir, 'RQ3')
    classifier_names = ['LR', 'RF', 'SC', 'KMEANS']
    paths = []
    for s in classifier_names:
        print(s)
        if s == 'LR' or s == 'RF':
            print(os.path.join(path2, s))
            paths.append(os.path.join(path2, s))
        elif s == 'SC' or s == 'KMEANS':
            paths.append(os.path.join(path3, s))
    print(paths)

    performance = [evalution_measure_lr, evalution_measure_rf, evalution_measure_sc, evalution_measure_kmeans]
...
def basic_experiment():
    ...
    evalution_measure = ['AUC', 'MCC', 'recall_effort', 'IFA_effort']
    evalution_measure_lr = np.zeros((len(evalution_measure), feature_lgorithm_num))
    evalution_measure_rf = np.zeros((len(evalution_measure), feature_lgorithm_num))
    ...
    for i in range(verification_num):
        ...
        train_data, train_label, test_data, test_label, test_idx = partition_dataset.partition(project_data, i, dataset_name)
        ...
        FS_name = ['UFSoL', 'UDFS', 'U2FS', 'SRCFS', 'SOGFS', 'SOCFS', 'RUFS', 'NDFS', 'LLCFS', 'Inf_FS2020', 'Inf_FS',
                'FSASL', 'FMIUFS', 'CNAFS', 'jelsr_lle', 'GLSPFS', 'LS', 'MCFS', 'NONE', 'REFS', 'AutoSpearman', 'SPEC', 'CFS', 'XGBF', 'RFF']
        # FS_name = ['LS']
        ...
        for fs_name in FS_name:
            ...
            if fs_name != 'CFS' and fs_name != 'XGBF' and fs_name != 'RFF':
            ...
            evalution_measure_lr = my_classifier.LR_classifier(train_label, train_data_sub, evalution_measure_lr)

            evalution_measure_rf = my_classifier.RF_classifier(train_label, train_data_sub, evalution_measure_rf)
        ...
        save_evalution_measure(evalution_measure_lr, evalution_measure_rf, evalution_measure_sc, evalution_measure_kmeans, evalution_measure)
