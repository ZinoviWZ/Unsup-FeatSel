'''输入：train_data、需要的特征数量 输出：s[0:k]'''''
import numpy as np
import scipy as sp
...
def refs(train_data,FeaNumCandi):
    X = train_data
    k = FeaNumCandi

    # mat = sp.io.loadmat('COIL20.mat')
    # X = mat['X']#<class 'numpy.ndarray'>
    ...
    while (i < k):
        ...
        for j in range(d):
            if (j not in S):
                Q_inv_j[j] = Q_inv[i] - (
                        gamma * Q_inv[i].dot(np.eye(d)[:, j:j + 1]).dot(np.eye(d)[j:j + 1, :]).dot(Q_inv[i])) / (
                                     1 + gamma * np.eye(d)[j:j + 1, :].dot(Q_inv[i]).dot(np.eye(d)[:, j:j + 1]))
    ...
    #     reconstructed = (gamma * pi_Xs + alpha * X_test).dot(np.linalg.inv(
    #         gamma * np.linalg.inv(X_test.T.dot(X_test)).dot(X_test.T.dot(pi_Xs)) + alpha * np.eye(len(L)) + betha * L))
